...
/**
 * Genera un reel de escenas encadenadas (lib/reel-secuencia.js).
 *
 *   node reel-secuencia.js reel-presupuestos.json
 *
 * El ritmo lo define el JSON: cada escena dice cuánto dura y el módulo las
 * encadena. La duración total no se declara en ningún lado — es la suma.
 */

const fs = require('fs');
const path = require('path');
const { execFileSync } = require('child_process');
const secuencia = require('./lib/reel-secuencia');
...
async function main() {
  const archivo = process.argv[2] || 'reel-presupuestos.json';
  ...
  fs.writeFileSync(pagina, secuencia.documento(g, marca), 'utf8');
  ...
  const dur = secuencia.duracion(g.escenas);
